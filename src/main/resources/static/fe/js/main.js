...
document.getElementById('submitOrder').addEventListener('click', function() {
    const selectedProducts = document.querySelectorAll('#cartTableBody tr');
  console.log(selectedProducts);
    // Tạo mảng chứa ID của các sản phẩm đã chọn
    const selectedProductIds = Array.from(selectedProducts).map(row => row.getAttribute('data-product-id')).join(',');

    // Gửi yêu cầu POST với tham số URL-encoded
    ...
    fetch('/user/create', {
     ...
     .then(response => {
            if (response.ok) {
                // Nếu yêu cầu thành công, chuyển hướng đến trang sản phẩm
